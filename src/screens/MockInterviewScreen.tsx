...
} from "../services/interviewApiSlice";
import type {
...
/** Route params forwarded by the Expo Router wrapper. */
interface MockInterviewRouteParams {
...
interface MockInterviewRoute {
  key?:    string;
  name?:   string;
  params?: MockInterviewRouteParams;
}

/** Component props. */
interface Props {
  route?:     MockInterviewRoute;
  navigation: NavigationBridge;
}

// ── Component ─────────────────────────────────────────────────────────────────

export default function MockInterviewScreen({ route, navigation }: Props) {
  const sessionTitle: string = route?.params?.sessionTitle || "Mock Interview";
  // Read candidateId from Redux; fall back to route params for backward compatibility.
  ...
  // RTK Query hooks — replace direct interviewApi.js imports
  const [triggerStartInterviewSession]    = useStartInterviewSessionMutation();
  const [triggerSubmitMockInterviewAudio] = useSubmitMockInterviewAudioMutation();
  ...
  const [questions,     setQuestions]     = useState<NormalizedInterviewQuestion[]>([]);
  const [questionIndex, setQuestionIndex] = useState<number>(0);
  const [statusText,    setStatusText]    = useState<string>("Preparing your mock session...");
  const [finalResult,   setFinalResult]   = useState<NormalizedInterviewResult | null>(null);

  const pulseAnim  = useRef(new Animated.Value(1)).current;
  const glowAnim   = useRef(new Animated.Value(0)).current;
  const mountedRef = useRef<boolean>(true);

  // Stores the epoch timestamp when recording started, used for elapsed-time calculation.
  const recordingStartTimeRef  = useRef<number | null>(null);
  const autoStopTriggeredRef   = useRef<boolean>(false);

  const currentQuestion: NormalizedInterviewQuestion | null =
  ...
  // Timer — polls Date.now() every 250 ms while recording.
  useEffect(() => {
    ...
    if (isRecording && recordingStartTimeRef.current) {
      interval = setInterval(() => {
        const elapsed = Math.floor(
          (Date.now() - (recordingStartTimeRef.current as number)) / 1000
        );
        setSeconds(elapsed);
      }, 250);
    }
  ...
  // Pulse / glow animation while recording.
  useEffect(() => {
  ...
  // Auto-stop when recording reaches AUTO_STOP_SECONDS.
  useEffect(() => {
  ...
  // Status text driven by elapsed time while recording.
  useEffect(() => {
    if (!isRecording || isSubmitting || sessionFinished) return;
  ...
  const speakCurrentQuestion = async (
    indexToSpeak: number = questionIndex,
    questionsList: NormalizedInterviewQuestion[] = questions
  ): Promise<void> => {
  ...
  const initializeSession = async (): Promise<void> => {
    try {
      ...
      const startResult = await triggerStartInterviewSession({
        candidateId,
      });
      ...
      // startResult.data is NormalizedInterviewSession (typed at slice level).
      const data = startResult.data;

      if (!mountedRef.current) return;

      const fetchedQuestions: NormalizedInterviewQuestion[] =
      ...
      logger.log("Mock session init error:", error);

      if (!mountedRef.current) return;

      Alert.alert("Error", (error as Error)?.message || "Could not start mock interview.");
  ...
  const handleStartRecording = async (): Promise<void> => {
    try {
      ...
      logger.log("Start recording error:", error);
  ...
  const handleFinishSession = async (isAutoFinish: boolean = false): Promise<void> => {
    if (!hasStartedRecording || !isRecording) {
      Alert.alert(
    ...
    try {
      ...
      const submitResult = await triggerSubmitMockInterviewAudio({
        audioUri,
        sessionId,
      });
      ...
      const resultQuery = await triggerGetMockInterviewResult(sessionId);
      if (resultQuery.error) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const errData = (resultQuery.error as any)?.data;
        throw new Error(
          errData?.detail || errData?.message ||
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          (resultQuery.error as any)?.error ||
          "Failed to fetch mock interview result."
        );
      }

      // resultQuery.data is NormalizedInterviewResult | undefined at the RTK Query level.
      // The error guard above already threw, so data is guaranteed non-undefined here.
      const result: NormalizedInterviewResult =
        resultQuery.data as NormalizedInterviewResult;
      ...
      logger.log("Finish session error:", error);
      ...
      Alert.alert(
        "Error",
        (error as Error)?.message || "Failed to complete mock interview."
      );
    }
  };
  ...
  const assistantStateText: string = isSubmitting
    ? "Processing..."
    : isSpeaking
    ? "Audio Playing..."
    : sessionFinished
              ...
              >
                ...
                <TouchableOpacity
                  ...
                  style={[
                    ...
                    (isSpeaking || isSubmitting || sessionFinished || questions.length === 0) &&
                      styles.micButtonDisabled,
                  ]}
                  onPress={handleStartRecording}
                  activeOpacity={0.9}
                  disabled={
                    isRecording || isSpeaking || isSubmitting || sessionFinished || questions.length === 0
                  }
                >
                  {isSubmitting ? (
