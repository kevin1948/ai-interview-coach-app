...
export default function useRealtimeWaveform() {
  ...
  async function start(): Promise<void> {
    try {
      logger.log("Waveform start: requesting permission");
      const permission = await AudioManager.requestRecordingPermissions();
      logger.log("Waveform start: permission =", permission);
      ...
      logger.log("Waveform start: creating recorder");
      const recorder = new AudioRecorder();

      logger.log("Waveform start: enabling file output");
      recorder.enableFileOutput();

      resetBars();

      logger.log("Waveform start: setting onAudioReady");
      recorder.onAudioReady(
        ...
        ({ buffer }: { buffer: any }) => {
          try {
            ...
            logger.log("Waveform processing error:", error);
          }
        }
      );
      ...
      logger.log("Waveform start: starting recorder");
      const result = recorder.start();
      logger.log("Waveform start: recorder result =", result);
      ...
      setIsRecording(true);
      logger.log("Waveform start: success");
    } catch (error) {
      logger.log("Recorder start error:", error);
  ...
  async function stop(): Promise<string | null> {
    try {
      logger.log("Waveform stop: called");
      ...
      try {
        ...
        logger.log("Waveform stop: clearOnAudioReady error:", error);
      }

      const result = await Promise.resolve(recorder.stop?.());
      logger.log("Waveform stop: result =", result);
      ...
      logger.log("Recorder stop error:", error);
      setIsRecording(false);
      resetBars();
      return audioFilePath;
    }
